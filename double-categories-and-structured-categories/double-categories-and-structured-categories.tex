...
\begin{translator}{Note from the translator}
  ...
  \todo incorporated comments/corrections from \unsure{citation for collected works} \todo and they're preceded by ``[Comm.]'' plus their comment number (unless they're just fixing typos, in which case they have been inserted silently); note that this means the footnotes in this version do not agree with those in the original (which we have instead placed in a bibliography); all numbers in the bibliography refer to their number in the collected works\todo
\end{translator}
...
\begin{itenv}{Proposition}
  ...
  The corresponding category $\mathscr{E}(I^\perp)$ \unsure{is this right?} of hypermorphisms \cite{47b,55} is a double category for the composition laws
  \[
    (f',g')\bullet(f,g)
    = (f'\bullet f,g)
  \]
