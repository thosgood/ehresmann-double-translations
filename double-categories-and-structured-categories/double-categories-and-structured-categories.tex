...
\begin{itenv}{Proposition}
  ...
  The corresponding category $\mathcal{E}(I^\perp)$ \unsure{is this right?} of hypermorphisms\footref{fn1} is a double category for the composition laws \unsure{is this right?}
  \[
    (f',g')\bullet(f,g)
    = (f'\bullet f,g)
  \]
...
\begin{itenv}{Corollary}
  Let $\CC$ be a double category;
  then $\CC^\bullet$ admits the longitudinal category $\vsq(\CC_0^\bullet,\CC_0^\perp)$ as a quotient category\footref{fn1}, where $\CC_0^\bullet$ (resp. $\CC_0^\perp$) is endowed with its structure as a subcategory of $\CC^\perp$ (resp. of $\CC^\bullet$).
\end{itenv}
