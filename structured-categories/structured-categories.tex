...
\begin{proof}
  ...
  \[
    ...
    \in\HH.
  \]
  The elements $p(S)p(s_1)$ and $(p(S)p(s))\bullet(p(s)p(s_1))$ are equal, since they are bounded above by $p(S)$ and have the same source and target ($p(s_1)$ and $p(S)$, respectively);
  thus
  \[
    \bar{g}
...
\begin{itenv}{Corollary 1}
  The conditions $s\subs_p S$, $s'\subs_p S$, and $p(s)=p(s')$ imply that $s=s'$.
\end{itenv}
