...
\todo turn longcomm into boxed env (border-left?), or ulcorner+hline and lrcorner+hline (or something similar)
...
\label{proposition:i-14}
  Suppose that $(\CC,p,\HH,\SS)$ satisfies Condition~(c) of \hyperref[theorem:i-3]{Theorem~3}, and that $s\relrhobar S$ in $(\CC,p,\HH,\SS)$ implies that $s<S$ in $\HH$;
...
\begin{proof}
  ...
  \begin{quote}
    ...
    \]
    Then there exists $h|s\in\HH$ such that
    \comm{49.1}{It is useful to notice that $h|s$ is unique. Indeed, its target is the unique $s'$ such that $s'<\beta(h)$ and $p(s')=\beta(p(h)|p(s))$ (because $p$ is strict), and its source $s$ and image $p(h)|p(s)$ are given.}
    \[
      \begin{aligned}
        h|s
        &< h,
  ...
  \[
    ...
    p(s_1)=p(s').
  \]
  By the above, we thus have that $s_1\subs_{p_u}s$, and so $s'=s_1<s$ by Corollary~1 of \hyperref[theorem:i-1]{Theorem~1}.
\end{proof}
...
\begin{longcomm}{50.3}
  ...
  \begin{itemize}
    ...
    \item In \hyperref[theorem:i-3]{Theorem~3}, Corollary~3 (of \hyperref[proposition:i-13]{Proposition~13}), and Propositions~\hyperref[proposition:i-13]{13} and \hyperref[proposition:i-14]{14}:
  ...
  Finally, in \hyperref[theorem:i-4]{Theorem~4}, ``$(\CC,<)$ is $(\JJ,\JJ',\JJ'')$-structured'' could be replaced by ``$(\CC,<)$ is $(\JJ^\mathcal{s},\JJ'{}^\mathcal{s},\JJ''{}^\mathcal{s})$-structured'', where $\JJ^\mathcal{s}$ is the category of sub-inductive maps, and $\JJ'{}^\mathcal{s}$ and $\JJ''{}^\mathcal{s}$ its intersections with $\widetilde{\Omega}'$ and $\widetilde{\Omega}''$ (respectively) (\cite{coll69}).
\end{longcomm}
