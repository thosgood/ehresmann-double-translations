...
\begin{proof}
  Indeed, if $[\bar{h}']\subs_{\sq p}[\bar{h}]$ then the proof of \hyperref{theorem:i-2} shows that we have $\alpha(\bar{h}')\subs_p\alpha(\bar{h})$, and it follows from \cref{proposition:i-6} that $\bar{h}'\subs_p\bar{h}$.
\end{proof}
...
\begin{examples}
  \begin{enumerate}
    \item
      In the category of homomorphisms $(\MM,\theta,\widetilde{\TT},\TT)$, a $\theta$-open homomorphism is an open continuous map.
...
\begin{proposition}{17}
  The conditions $s\subs S$ in $(\FF,\bar{p},\bHH,\bar{\Gamma})$ and $s\subs S$ in $(\MM,p_\FF\bar{p},\bHH,\bar{\Gamma})$ are equivalent;
  $(\FF,\bar{p},\bHH,\bar{\Gamma})$ is right solving if and only if $(\MM,p_\FF\bar{p},\bHH,\bar{\Gamma})$ is right solving.
\end{proposition}
