\documentclass[a4paper,oneside,nobib,nofonts,notitlepage,notoc,nols,fleqn,justified]{tufte-book}
...
\newcommand{\comm}[3][0em]{{\normalmarginpar\marginnote[#1]{[#2] #3}}}
...
\comm[-2em]{22.1}{A faithful set-valued functor is right solving if and only if it creates canonical equalisers (cf. Comment~221.2 on \cite{coll100}).}%
...
\begin{rmenv}{Remarks}
  ...
  \begin{enumerate}
    ...
    \label{remark:i-3}
      ...
      \comm[-2em]{25.2}{The set-valued functor associated to $(\CC^\bullet,\beta,\alpha^{-1}(e))$ is the partial Hom functor $\Hom(e,-)\colon\CC\to\mathsf{Set}$.}%
  \end{enumerate}
\end{rmenv}
...
this functor $\bar{F}$, which is identical to $(\MM,\bar{F}_a,\CC')$, is representable.\comm[-2em]{26.1}{Regarding $\CC'$, Bénabou gives the following criterion: $F\colon\CC\to\SS$ is representable if and only if $\CC\hookrightarrow\CC'$ has a left adjoint \cite{comm5}.}
...
\begin{proof}
  ...
  Condition~(2) implies that $p$ belongs to $\widetilde{\Omega}'$ (see \hyperref[section:ii.6]{§II.6}), i.e. (§IV) that $(\HH,\subs)$ is an ordered category over $(\CC,<)$.\comm[-3em]{35.1}{Charles never published §IV, but the results that he intended to put in it are scattered across several papers, e.g. ordered categories over an ordered category are considered in \cite{coll75}.}
\end{proof}
...
In what follows, we further suppose that $\HH$ is endowed with an order relation $<$ such that $(\HH,<)$ is an inductive category, and such that $p$ is a strict inductive map from $(\HH,<)$ to $(\CC,<)$, i.e. such that $(\HH,<)$ is an inductive category over $(\CC,<)$ with respect to $p$ (see \hyperref[section:iv]{§IV}).\comm[-1em]{42.1}{This notion (of an inductive category over $(\CC,<)$ with respect to $p$) is considered in \cite{coll53}. The definition of a \emph{strict inductive map} is given in Part~II, Section~6.}
